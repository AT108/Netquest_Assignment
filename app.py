...
def find_by_id(id):
    ...
    if name_and_picture:
        ...
        return jsonify({'message': 'Name and picture with given id ' + id + ' not found'}), 404
...
def create():
    try:
        if request.method == 'POST':
            ...
            db.session.add(obj)
            db.session.commit()
        return jsonify({'message': 'Name and picture created successfully', 'obj': obj}), 201
...
def update():
    try:
        if request.method == 'PUT':
            data = request.json
            if data.get('id') is None or data.get('title') is None or data.get('img') is None:
...
class NetquestAssignmentTestCase(unittest.TestCase):
    ...
    def test01_create(self):
        # given, when
        response = self.app.post('/v1/api/create', json={'title': 'Test 1 - test case object',
    ...
    def test02_update(self):
        # given, when
        response = self.app.put('/v1/api/update', json={'id': self.__class__.test_name_and_picture_id,
    ...
    def test04_find_by_id(self):
        # given, when
        response = self.app.get('/v1/api/find-by-id/' + self.__class__.test_name_and_picture_id)
        data = json.loads(response.data.decode())
        # then
        self.assertEqual(response.status_code, 200)
    ...
    def test05_delete_by_id(self):
        # given, when
        response = self.app.delete('/v1/api/delete-by-id/' + self.__class__.test_name_and_picture_id)
        data = json.loads(response.data.decode())
        # then
        self.assertEqual(response.status_code, 200)
        self.assertEqual(self.__class__.test_name_and_picture_id, data['id'])
